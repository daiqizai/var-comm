"""Verify new R3 physical observations/features and every inherited source image on CPU."""
...
from vector_control.evaluation import new_names
...
def audit_saved(root, rows, clean, noiseless, support, config, base, references):
    ...
    by_source = {index: [] for index in range(len(identifiers))}
        ...
        with np.load(directory / 'receiver_features.npz', allow_pickle=False) as features:
            for row in by_source[index]:
                if row['image_id'] != identifier:
                    raise RuntimeError('R3 archived row belongs to a different source')
                max_psnr = max(max_psnr, check_image(cache.image(row), source, row))
                count += 1
                if row.get('arm') in names:
                    if row['image_archive'] != str(directory / 'reconstructions.npz') or row['r3_quality_origin'] != 'new_r3_measurement':
                        raise RuntimeError('new R3 image does not point to its actual measured archive')
                    ...
                    feature_count += 1
        print(f'R3 saved quality CPU audit {index + 1}/{len(identifiers)}', flush=True)
    return {'status': 'R3_SAVED_QUALITY_CPU_AUDIT_PASS', 'checked_image_rows': count, 'checked_new_feature_rows': feature_count,
