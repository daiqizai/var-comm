"""Replay the selected R3 and fifteen registered references in one separate timing phase."""
...
sys.path[:0] = [str(EXPERIMENT / 'src'), str(JOINT / 'scripts'), str(BASE / 'scripts')]
...
from vector_control.evaluation import audited_endpoint, evaluation_output, learned_names, load_evaluation
from vector_control.references import ImageCache, References, read_rows
from vector_control.timing import all_models, archive_origin, measurement_order, summarize, validate_config, waveform_key
...
def main():
    ...
    if arguments.step != evaluation['required_total_updates']:
        raise ValueError('R3 timing requires the complete10000 opportunity')
    if not arguments.execute:
        print('PLAN ONLY: fixed32 sources, five SNRs, seed2001, sixteen frozen models; no quality reselection')
        return
    configure_torch()
    ...
    validate_receipt(quality_receipt, 'R3_QUALITY_COMPLETE', {'r3_milestone_sha256': sha256(milestone_path),
        'r3_review_sha256': sha256(review_path), 'reference_quality_sha256': references.receipt_sha})
    output = evaluation_output(evaluation, 'timing')
    if (output / 'completion.json').exists():
        raise RuntimeError('R3 timing is already complete')
    bindings = {'r3_milestone_sha256': sha256(milestone_path), 'quality_receipt_sha256': sha256(quality_receipt),
    ...
    if arguments.resume:
        ...
        if any(metadata[key] != value for key, value in bindings.items()):
            raise RuntimeError('R3 timing resume changed frozen quality or selection')
    else:
        ...
        files = [Path(__file__), EXPERIMENT / 'configs/evaluation.yaml', EXPERIMENT / 'docs/evaluation_protocol.md',
            *sorted((EXPERIMENT / 'src/vector_control').glob('*.py')),
            EXPERIMENT.parent / 'wetok-innovation-r1/src/innovation_comm/inference.py', BASE / 'scripts/evaluate_interfaces.py']
    ...
    try:
        status('LOADING_SEPARATE_R3_TIMING')
        require_uncontended_gpu()
        device = torch.device('cuda:0')
        parent, models, choices = all_models(config, r2, original, grid, reference, base, milestone, device)
        ...
        if before['native'] != references.frozen_models['native']:
            raise RuntimeError('R3 timing changed the visual codec')
        ...
        archive_roots = {'r3': quality, 'r2': references.root, 'grid': references.previous.root, 'joint': references.previous.previous.root}
        for position, index in enumerate(evaluation['timing_source_indices']):
            ...
            if source_receipt.exists():
                ...
                if record['image_id'] != identifiers[index]:
                    raise RuntimeError('R3 timing source changed during resume')
                for relative, expected in record['output_hashes'].items():
                    if sha256(directory / relative) != expected:
                        raise RuntimeError('a committed R3 timing source changed')
            ...
            if not torch.equal(encoded.cpu(), torch.from_numpy(np.array(codes[index:index + 1, 0], copy=True))):
                raise RuntimeError('R3 timing tokenization differs from the source cache')
                ...
                for snr_position, snr in enumerate(evaluation['timing_snrs_db']):
                    ...
                    for order_index, name in enumerate(measurement_order(names, frame)):
                        ...
                        if choices[name]['checkpoint_sha256'] != expected['checkpoint_sha256'] or encoder_hashes[name] != expected['encoder_sha256']:
                            raise RuntimeError('R3 timing model differs from its quality selection')
                        ...
                        if digest(saved_signal) != expected['transmitted_sha256'] or digest(saved_received) != expected['received_sha256']:
                            raise RuntimeError('R3 timing changed the saved quality waveform')
                        if expected['noise_sha256'] != digest(raw_noise(identifiers[index], seed)):
                            raise RuntimeError('R3 timing noise pairing changed')
                        ...
                        if signal_error > tolerance['frozen_signal_max_error']:
                            raise RuntimeError('R3 timing transmitter does not replay its quality waveform')
                        torch.cuda.synchronize()
                        tick = time.perf_counter()
                        ...
                        if image_error > tolerance['frozen_pixel_max_error'] or digest(received) != expected['received_sha256']:
                            raise RuntimeError('R3 timing receiver output or observation differs from quality')
            ...
            status('MEASURING_SEPARATE_R3_TIMING')
            print(f'R3 timing {completed}/32 rows={len(rows)}', flush=True)
        summary, paired = summarize(rows, evaluation, names, base, config)
        if len(paired) != evaluation['timing_paired_intervals']:
            raise RuntimeError('R3 timing lost a registered comparison')
        if before != {name: module_sha256(model) for name, model in objects.items()}:
            raise RuntimeError('R3 timing changed model parameters')
        verify_sources(metadata['source_hashes'])
        write_rows(output / 'per_frame.csv', rows)
        write_rows(output / 'summary.csv', summary)
        write_rows(output / 'paired.csv', paired)
        status('R3_TIMING_COMPLETE_NOT_RESEARCH_COMPLETE')
        sessions = [json.loads(path.read_text()) for path in sorted((output / 'sessions').glob('*.json'))]
        terminal = {'R3_TIMING_COMPLETE_NOT_RESEARCH_COMPLETE', 'R3_TIMING_FAILED_OR_INTERRUPTED'}
        write_json(output / 'completion.json', {'status': 'R3_TIMING_COMPLETE', 'completed_local': now(), **bindings,
        ...
        status('R3_TIMING_FAILED_OR_INTERRUPTED', error=repr(error))
        raise
