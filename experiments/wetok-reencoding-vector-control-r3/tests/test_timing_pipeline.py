"""Actual R3 timing driver: four waveform origins, fixed quality outputs, and resource-guard resume."""
...
from evaluation_fixtures import CPUTorch, ModelFixture, NativeFixture, driver, fixture
...
from vector_control.evaluation import learned_names
from vector_control import timing
...
class TimingPipelineTests(unittest.TestCase):
    def test_real_timing_driver_replays_all16_and_does_not_redo_committed_sources(self):
        ...
        evaluation.update(timing_source_indices=[0, 1], timing_snrs_db=[1.], timing_paired_intervals=45)
        ...
        with TemporaryDirectory() as temporary:
            ...
            roots = {'r3': quality, 'r2': root / 'old_R2', 'grid': root / 'old_grid', 'joint': root / 'old_joint'}
            rows = []
            ...
            (quality / 'completion.json').write_text(json.dumps({'status': 'R3_QUALITY_COMPLETE',
                'r3_milestone_sha256': sha256(milestone_path), 'r3_review_sha256': sha256(review_path),
                'reference_quality_sha256': 'sealed_R2_receipt', 'source_hashes': {},
            ...
            class ReferencesFixture:
                ...
                def __init__(self, *args):
                    self.root = roots['r2']
                    self.previous = SimpleNamespace(root=roots['grid'], previous=SimpleNamespace(root=roots['joint']))
            ...
            self.assertEqual(completion['rows'], 32)
