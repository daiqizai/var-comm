...
def main():
    ...
    if not (OUT/'launch_identity_v2.json').exists():
        write_json(OUT/'launch_identity_v2.json',{'head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'pid':os.getpid(),'time':time.time(),'git_status':subprocess.check_output(['git','status','--porcelain'],cwd=ROOT,text=True),'scope':'source A and new budget first20k; not whole supplement completion'})
    plan=[
    ...
    identity={'commands':[c for c,_ in plan],'bindings':bindings([__file__,EXP/'scripts/run_budgets_after_retirement.sh',Path(__file__).with_name('retired_gate.py'),Path(__file__).with_name('coordinator.py'),CONFIG]),'retired_predecessor':{'receipt':gate['retirement_receipt'],'sha256':gate['retirement_receipt_sha256']},'cache_completion':gate['required_cache_completion']}
    register(OUT/'coordinator_identity_v2.json',identity)
