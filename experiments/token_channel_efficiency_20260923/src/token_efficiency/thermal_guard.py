...
def signal_verified(expected):
    ...
    if ' -m token_efficiency.coordinator_v2 ' not in actual['cmdline']:raise RuntimeError('not the owned budget coordinator')
...
def active_owned_queue():
    ...
    for path in Path('/proc').iterdir():
        ...
        try:
            ...
            if b'-m' in argv and argv[argv.index(b'-m')+1] in (b'token_efficiency.coordinator',b'token_efficiency.coordinator_v2',b'token_efficiency.budget_train'):
...
def main():
    ...
    while not stopping[0]:
        ...
        with (run/'controller.log').open('w') as f:
            process=subprocess.Popen(['bash',str(EXP/'scripts/run_budgets_after_retirement.sh')],cwd=ROOT,stdin=subprocess.DEVNULL,stdout=f,stderr=subprocess.STDOUT,start_new_session=True)
        # The shell immediately execs Python; capture that final command identity.
        expected=None
        for _ in range(50):
            p=proc_identity(process.pid)
            if p and ' -m token_efficiency.coordinator_v2 ' in p['cmdline']:expected=p;break
