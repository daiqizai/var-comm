"""Versioned microbatch execution; original registration remains historical, runtime identity is additive."""
...
from .microbatch_runtime import qualify_microbatch
from .budget_lifecycle import register_extension,validate_extension_resume

SEED=2026092304

def main():
    p=argparse.ArgumentParser();p.add_argument('--N',type=int,choices=[2048,3060],required=True);p.add_argument('--qualification-only',action='store_true');p.add_argument('--plan',required=True);a=p.parse_args()
    configure_runtime();require_available();device=torch.device('cuda:0');cfg=config()['continuous'];spec=read(a.plan);micro=spec['microbatch'];until=spec['until']
    if spec['N']!=a.N:raise RuntimeError('plan budget')
    if cfg['batch_size']!=16:raise RuntimeError('effective batch changed')
    ...
    ids=torch.arange(16);b=cal.batch(ids,torch.zeros_like(ids),torch.zeros_like(ids),[4101]*16,device)
    ...
    probe_out=Path(a.plan).parent;probe_out.mkdir(parents=True,exist_ok=True)
    if micro==8:qualify_microbatch(twins,b,decoder,lp,scale,probe_out,micro)
    else:qualify(twins,b,decoder,lp,scale,probe_out)
    del twins,b
    ...
    files=[CONFIG,Path(__file__).with_name('budget_train.py'),Path(__file__).with_name('models.py'),Path(__file__).with_name('common.py'),old/'train.py',old/'data.py',ROOT/'experiments/var-latent-enhancement-20260917/research/src/latent_research/models.py',EXP/'quality_targets.json']
    ...
    execution=register_extension(out,spec,Path(a.plan),regsha,Path(__file__))
    ...
    if (out/'latest.json').exists():
        ...
        validate_extension_resume(payload,execution,spec,checked_checkpoint(read(out/'latest.json'),ROOT))
    ...
    def checkpoint(reason):
        ...
        path=out/'checkpoints'/f"step_{state['step']:05d}_{reason}_{spec['version']}.pt"
        save_torch(path,{'models':models.state_dict(),'optimizers':{n:o.state_dict() for n,o in opts.items()},'order':order.state_dict(),'rng':rng.get_state(),'torch_rng':torch.get_rng_state(),'cuda_rng':torch.cuda.get_rng_state_all(),'state':state,'registration_sha256':regsha,'execution_identity':execution})
        ...
        for n,rec in state['selection'].items():
            if rec and rec['step']==state['step']:write_json(out/f'selected_{n}.json',{**rec,'checkpoint':str(path),'checkpoint_sha256':sha,'arm_key':n,'registration_sha256':regsha,'total_updates':state['updates'][n],'parent_updates':0,'N':a.N,'execution_identity':execution})
    ...
    try:
        ...
        while state['step']<until:
            ...
            b=train.batch(ids,si,ni,[registration['channel_seed']+state['step']]*len(ids),device);started=time.time();update(models,opts,b,decoder,lp,scale,micro)
        ...
        verify_snapshot(read(execution['path'])['bindings']);verify_snapshot(registration['bindings']);verify_snapshot(train.bindings);verify_snapshot(cal.bindings)
        if state_sha256(decoder)!=registration['decoder_sha256']:raise RuntimeError('frozen Dc changed')
        write_json(Path(a.plan).parent/'completion.json',{'status':'CALIBRATION_AUTHORIZED_EXTENSION_COMPLETE_NOT_CONVERGENCE','state':state,'selected':{n:read(out/f'selected_{n}.json') for n in models},'registration_sha256':regsha,'pending':'calibration decision, selected development and shared online timing; no new holdout','synthetic':False,'execution_identity':execution})
