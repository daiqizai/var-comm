...
from .microbatch_runtime import qualify_microbatch, runtime_registration, validate_execution_checkpoint

SEED=2026092304

def main():
    p=argparse.ArgumentParser();p.add_argument('--N',type=int,choices=[2048],required=True);p.add_argument('--qualification-only',action='store_true');a=p.parse_args()
    configure_runtime();require_available();device=torch.device('cuda:0');cfg=config()['continuous'];spec=read(EXP/'microbatch_runtime.json');micro=spec['budgets'][str(a.N)]
    if cfg['batch_size']!=spec['effective_batch']:raise RuntimeError('effective batch changed')
    ...
    probe_out=out/'execution'/spec['version'];probe_out.mkdir(parents=True,exist_ok=True)
    qualify_microbatch(twins,b,decoder,lp,scale,probe_out,micro);del twins,b
    ...
    execution=runtime_registration(out,spec,regsha,Path(__file__))
    ...
    if (out/'latest.json').exists():
        ...
        validate_execution_checkpoint(payload,execution,spec,checked_checkpoint(read(out/'latest.json'),ROOT))
