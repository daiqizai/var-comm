...
import argparse, copy, hashlib, json, time, signal
...
from latent_followup.run_identity import selection_record, checked_checkpoint, register_run, load_resume
...
def selected_record(p):
 r=read(p);c=checked_checkpoint(r,VAR)
...
def calibrate_v2(arms,predictor,decoder,perceptual,population,scale,out,step,cfg,device,phase='calibration'):
 ...
 write_json(Path(out)/phase/('_'.join(arms)+f'_full_{step:05d}.json'),{'step':step,'phase':phase,'rows':rows,'summary':summary,'elapsed_seconds':time.perf_counter()-started});return summary
def main():
 ...
 while state['step']<int(cfg['updates']['maximum']) and not(state['step']>=int(cfg['updates']['minimum']) and all(v>=int(cfg['plateau_checks']) for v in state['plateau_checks'].values())):
  ...
  if stop_requested[0]:
   checkpoint('safe_boundary_pause');write_json(out/'status.json',{'status':'PAUSED_AT_PAIRED_UPDATE','step':state['step']});return
 ...
 for n,r in state['selection'].items():
  path=out/'checkpoints'/f"step_{int(r['step']):05d}.pt";selected[n]=selection_record(n,r,path,state['parent_step'])
 selected_evaluation={}
 for n,r in selected.items():
  checked_checkpoint(r,VAR)
