"""Matched geometry quality and timing, with real old-control image replay."""
...
from wetok_comm.geometry_evaluation import GeometryReferences, geometry_supplement_statistics, load_geometry_evaluation, method_definitions, model_registry, system_reference_names, validate_geometry_rows
...
def main():
    ...
    evaluation, study, base, qualification = load_geometry_evaluation()
    if arguments.total != evaluation['matched_total_updates']:
        raise ValueError('development comparison is registered only at the matched total7000/image5000 budget')
    definitions = method_definitions(study)
    if not arguments.execute:
        print(json.dumps({'plan_only': True, 'fresh_geometry_models': 6, 'system_references': 4,
                          'main_rows': 21000, 'fixed_support_rows': 600, 'matched_image_budget': 5000}, indent=2))
        return
    require_uncontended_gpu()
    configure_torch()
    deep_config = load_deep_support()
    training = geometry_output(study, 'training')
    ...
    if milestone['status'] != 'GEOMETRY_MILESTONE_COMPLETE' or milestone['total_updates_per_new_arm'] != arguments.total:
        raise RuntimeError('geometry training milestone is incomplete')
    verify_sources(milestone['source_hashes'])
    review_path = geometry_output(study, 'analysis') / f'calibration_total_{arguments.total:07d}/completion.json'
    ...
    if review['data_noise_phase_Adam_power_selection_audit'] != 'PASS' or review['milestone_sha256'] != sha256(milestone_path):
        raise RuntimeError('matched geometry calibration/selection has not been audited')
    output = (arguments.output_dir or geometry_output(study, 'evaluation') / f'total_{arguments.total:07d}').resolve()
    if not output.is_relative_to((PROJECT / 'outputs').resolve()) or (output / 'completion.json').exists():
        raise ValueError('output escapes project or overwrites a frozen result')
    if arguments.resume:
        ...
        if metadata['milestone_sha256'] != sha256(milestone_path) or metadata['calibration_review_sha256'] != sha256(review_path):
            raise RuntimeError('matched milestone or calibration review changed during evaluation')
    else:
        ...
        metadata = {'created_local': now(), 'milestone_sha256': sha256(milestone_path), 'calibration_review_sha256': sha256(review_path),
            ...
            'control_qualification_sha256': study['control_qualification_sha256'],
            'control_milestone_sha256': study['control_milestone_sha256'],
            'deep_support_reference_sha256': deep_config['reference_receipt_sha256'],
            'source_hashes': snapshot(output, [Path(__file__), EXPERIMENT / 'scripts/evaluate_interfaces.py',
                EXPERIMENT / 'scripts/train_milestone.py', EXPERIMENT / 'configs/geometry_evaluation.yaml',
                EXPERIMENT / 'configs/geometry_study.yaml', EXPERIMENT / 'docs/geometry_evaluation_protocol.md',
                EXPERIMENT / 'configs/deep_support.yaml', *[WORKSPACE / relative for relative in deep_config['source_hashes']],
    ...
    try:
        status('LOADING_SELECTED_GEOMETRY_MODELS')
        references = GeometryReferences(study)
        ...
        networks, selections = model_registry(study, base, qualification, milestone, training, device)
        ...
        maximum_power_error, maximum_support_pixel_error, maximum_control_pixel_error = 0., 0., 0.
        ...
        for index, identifier in enumerate(identifiers):
            ...
            if (directory / 'receipt.json').exists():
                ...
                maximum_control_pixel_error = max(maximum_control_pixel_error, saved['maximum_control_pixel_error'])
            ...
            waveforms, features = {}, {}
            local_power, local_support_pixel_error, local_control_pixel_error = 0., 0., 0.
            ...
            for snr in base['evaluation']['snrs_db']:
                ...
                for seed in base['evaluation']['noise_seeds']:
                    ...
                    for name, network in networks.items():
                        ...
                        control_pixel_error = ''
                        if definitions[name]['geometry'] == '153x40':
                            original_image, original_row = references.control_image(index, identifier, snr, seed, definitions[name]['variant'], noise_hash)
                            control_pixel_error = float((image[0].cpu() - original_image).abs().max())
                            if control_pixel_error > evaluation['control_pixel_replay_max_error']:
                                raise RuntimeError('remeasured geometry control does not reproduce its frozen image')
                            local_control_pixel_error = max(local_control_pixel_error, control_pixel_error)
                            expected_scores[name, snr, seed] = original_row
                        ...
                        local_rows.append({**common, 'arm': name, 'geometry': definitions[name]['geometry'], 'variant': definitions[name]['variant'],
                            'available_image_updates': 5000, 'representation_updates': 2000, 'control_replay_pixel_error': control_pixel_error,
                            'communication_parameters': sum(value.numel() for value in network.parameters()),
                            'header_uses': 0, 'data_uses': 3060, 'raw_bits': 8192,
                            ...
                            'selected_image_step': selections[name]['step'],
                            'selected_global_data_step': study['parent_step'] + selections[name]['step'],
                            'transmitted_sha256': hashlib.sha256(signal_array.tobytes()).hexdigest()})
                    for name in system_reference_names(study):
                        ...
                        local_rows.append({**common, 'arm': name, 'geometry': '', 'variant': '',
                            'available_image_updates': '', 'representation_updates': '', 'control_replay_pixel_error': '', 'communication_parameters': '',
                            'header_uses': int(previous['header_uses']),
                            ...
                            'latency_source': 'historical_not_a_latency_ranking', 'selected_image_step': '',
            ...
            for row in local_rows + local_noiseless + local_support:
                ...
                if row.get('image_store') == 'frozen_reference' or (row.get('geometry') == '153x40' and 'snr_db' in row):
            ...
            write_json(directory / 'receipt.json', {'image_id': identifier, 'maximum_power_error': local_power,
                'maximum_support_pixel_error': local_support_pixel_error, 'maximum_control_pixel_error': local_control_pixel_error,
            ...
            maximum_control_pixel_error = max(maximum_control_pixel_error, local_control_pixel_error)
            completed_images = index + 1
            write_csv(output / 'per_frame.csv', rows)
            status('EVALUATING_GEOMETRY')
            print(f'geometry development {completed_images}/100 rows={len(rows)}', flush=True)
        validate_geometry_rows(rows, study, base)
        geometry_supplement_statistics(rows, support_rows, study, base)
        ...
        status('GEOMETRY_EVALUATION_COMPLETE_NOT_RESEARCH_COMPLETE')
        ...
        terminal = {'GEOMETRY_EVALUATION_COMPLETE_NOT_RESEARCH_COMPLETE', 'GEOMETRY_EVALUATION_FAILED_OR_INTERRUPTED'}
        write_json(output / 'completion.json', {'status': 'GEOMETRY_EVALUATION_COMPLETE', 'completed_local': now(),
            ...
            'maximum_control_pixel_error': maximum_control_pixel_error,
        ...
        status('GEOMETRY_EVALUATION_FAILED_OR_INTERRUPTED', error=repr(error))
        raise
