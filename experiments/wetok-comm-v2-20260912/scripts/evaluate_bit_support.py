"""Matched recipe continuation evaluation with frozen pre-repair best references."""
...
from wetok_comm.bit_support import load_repair, arm_definitions
from wetok_comm.recipe_evaluation import evaluation_config, RecipeReferences
...
def main():
    ...
    config = evaluation_config(repair, base)
    ...
    if milestone['status'] != 'REPAIR_MILESTONE_COMPLETE' or milestone['additional_updates_per_arm'] != arguments.milestone:
        raise RuntimeError('milestone is not complete')
    verify_sources(milestone['source_hashes'])
    preserved = training / milestone['optimizer_checkpoint']
    if not preserved.exists() and sha256(training / 'resume.pt') == milestone['optimizer_checkpoint_sha256']:
        os.link(training / 'resume.pt', preserved)
    if preserved.exists():
        if sha256(preserved) != milestone['optimizer_checkpoint_sha256']:
            raise RuntimeError('preserved model/optimizer milestone checkpoint changed')
        write_json(training / 'milestones' / f'additional_{arguments.milestone:07d}_optimizer.json',
                   {'checkpoint': str(preserved.relative_to(training)), 'sha256': milestone['optimizer_checkpoint_sha256'],
                    'additional_updates_per_arm': arguments.milestone, 'storage': 'hard_link_no_large_asset_copy'})
    output = (arguments.output_dir or output_path(config, 'evaluation') / f'additional_{arguments.milestone:07d}').resolve()
    ...
    if arguments.resume:
        ...
        sources = snapshot(output, [Path(__file__), arguments.config, EXPERIMENT / repair['base_config'], EXPERIMENT / 'docs/protocol.md', EXPERIMENT / 'docs/bit_support_protocol.md',
    ...
    for arm in config['arms']:
        ...
        network = new_network(base, definitions[arm]['variant'], device)
    ...
    references = RecipeReferences(base, repair)
    modem = WeTokDigital()
    rows, clean_rows, diagnostic_rows = [], [], []
    controls = ['digital_m8', 'digital_adaptive', 'perceptual_deepjscc'] + ['old_selected__' + variant for variant in repair['variants']]
    max_power_error = 0.
            ...
            for seed in config['evaluation']['noise_seeds']:
                ...
                for arm, network in networks.items():
                    ...
                    local_rows.append({**common, 'arm': arm, 'header_uses': 0, 'data_uses': 3060, 'raw_bits': 8192,
                        ...
                        'selected_step': milestone['selected'][arm]['global_step']})
                ...
                for arm in controls:
                    ...
                    source_bits = 8192 if arm.startswith('old_selected__') else ('' if arm == 'perceptual_deepjscc' else (3060 if arm == 'digital_m8' else (1860 if snr < 2.5 else 3060 if snr < 5.5 else 5088)))
                    local_rows.append({**common, 'arm': arm, 'header_uses': int(legacy['header_uses']), 'data_uses': int(legacy['data_uses']),
                        'raw_bits': source_bits, 'coded_bits': '' if arm == 'perceptual_deepjscc' or arm.startswith('old_selected__') else 6120,
    ...
    expected = 100 * 7 * 3 * (len(config['arms']) + 1 + len(controls))
    ...
    print('RECIPE_DEVELOPMENT_EVALUATION_COMPLETE', flush=True)
