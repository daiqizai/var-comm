"""Small CPU fixtures exercise the real training/resume driver, not scientific performance."""
...
from joint_sender.common import initial_system
from joint_sender.runtime import calibration
...
from test_joint_sender import ToyDecoder, ToyPerceptual

spec = importlib.util.spec_from_file_location('joint_sender_training_fixture', EXPERIMENT / 'scripts/train.py')
...
class TrainingPipelineTests(unittest.TestCase):
    def test_calibration_transmits_with_the_updated_system_not_the_frozen_parent(self):
        system = object()
        with mock.patch('innovation_comm.runtime.calibrate', return_value=[]) as implementation:
            calibration(system, 'population', 'decoder', 'lpips', 'reference', 'base', 'cpu')
            self.assertIs(implementation.call_args.args[0], system)
            self.assertIs(implementation.call_args.args[1], system)

    def test_real_driver_resume_matches_uninterrupted_joint_updates(self):
        ...
        config['full_calibration_steps'], config['monitor_every'] = [0, 1, 2], 1
        ...
        with TemporaryDirectory() as temporary:
            ...
            (control_root / 'milestones').mkdir(parents=True)
            control, review = control_root / 'milestones/control.json', root / 'review.json'
            control.write_text('{}')
            review.write_text('{}')
            (control_root / 'initialization.json').write_text('{}')
            for name in config['variants']:
                (control_root / name).mkdir()
                (control_root / name / 'training.csv').write_text('synthetic_control\n')
                for step in config['full_calibration_steps']:
                    (control_root / name / f'full_{step:07d}.csv').write_text('synthetic_calibration_control\n')
            parent = parent_factory(reference, base, 'cpu')
            decoder = visual_factory('cpu', 'decoder')
            perceptual = ToyPerceptual()
            profile = {'status': 'JOINT_IMAGE_GRADIENT_PROFILE_PASS', 'optimizer_updates': 0, 'source_hashes': {},
            ...
            with ExitStack() as stack:
                patches = {'torch': CPUTorch(), 'configure_torch': lambda: None, 'require_uncontended_gpu': lambda: None,
                    'settings': lambda: (config, reference, base, {}), 'complete_control': lambda cfg, ref: ({}, control, review),
                    'load_parent': parent_factory, 'FrozenWeTok': visual_factory, 'load_lpips': lambda device: ToyPerceptual(),
                ...
                with mock.patch.object(sys, 'argv', ['train.py', '--until-update', '1', '--execute']):
                    trainer.main()
                first_milestone = paths['training'] / 'milestones/step_0000001_optimizer.pt'
                first_hash = sha256(first_milestone)
                ...
                self.assertEqual(sha256(first_milestone), first_hash)
            ...
            self.assertEqual(resumed['completed_joint_updates'], 2)
            ...
            for name in config['variants']:
                ...
                for row, reference_row in zip(resumed['histories'][name], uninterrupted['histories'][name]):
                    self.assertEqual(row['received_sha256'], reference_row['received_sha256'])
                    self.assertEqual(row['loss'], reference_row['loss'])
