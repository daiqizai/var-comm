"""New Grid quality only, retaining all sealed references and never treating shared runtime as latency."""
...
from grid_controls.evaluation import audited_grid, load_evaluation, model_registry, new_names, validate_diagnostics, validate_reference_diagnostics, validate_rows, validate_selection
from grid_controls.hardware import admit_quality, quality_telemetry
from grid_controls.references import References, project_reference, read_rows
...
def main():
    ...
    if arguments.step != 5000:
        raise ValueError('quality evaluation requires matching5000 opportunities')
    if not arguments.execute:
        print('PLAN ONLY: 4200 new Grid rows +33600 frozen rows, no current latency; no training or new holdout')
        return
    configure_torch()
    milestone, milestone_path, review_path, original_bindings = audited_grid(config, original, reference, 5000)
    references = References(evaluation, config, original, reference)
    output = output_path(config, 'evaluation') / 'quality_0005000'
    if (output / 'completion.json').exists():
        raise RuntimeError('quality evaluation is already complete; do not overwrite')
    bindings = {'grid_milestone_sha256': sha256(milestone_path), 'grid_review_sha256': sha256(review_path),
        'original_joint_evaluation_sha256': references.receipt_sha, 'original_joint_analysis_sha256': original_bindings['hashes']['joint_analysis'],
        'source_tensor_identity_table_sha256': references.source_table_sha}
    if arguments.resume:
        ...
        if any(metadata[key] != value for key, value in bindings.items()) or metadata['shared_start_requested'] != arguments.shared_gpu:
            raise RuntimeError('quality resume changed selection, references or admission mode')
    else:
        output.mkdir(parents=True, exist_ok=False)
        files = [Path(__file__), EXPERIMENT / 'configs/evaluation.yaml', EXPERIMENT / 'docs/evaluation_protocol.md',
            *sorted((EXPERIMENT / 'src/grid_controls').glob('*.py')), *sorted((JOINT / 'src/joint_sender').glob('*.py')),
    ...
    try:
        status('QUALIFYING_GRID_QUALITY_GPU_ADMISSION')
        admission = admit_quality(evaluation, arguments.shared_gpu)
        write_json(output / 'admissions' / (session_path.stem + '.json'), admission)
        ...
        if any(before[key] != references.frozen_models[key] for key in ('native', 'lpips', 'dino')):
            raise RuntimeError('visual codec or metric models differ from the frozen references')
        ...
        for index, identifier in enumerate(identifiers):
            ...
            if source_receipt.exists():
                saved = json.loads(source_receipt.read_text())
                if saved['image_id'] != identifier:
                    raise RuntimeError('resumed quality source identity differs')
                for relative, expected in saved['output_hashes'].items():
                    if sha256(directory / relative) != expected:
                        raise RuntimeError('committed quality source artifact changed')
            ...
            if not torch.equal(actual_codes, truth_codes):
                raise RuntimeError('source tokenization differs from the frozen cache')
            truth = indices_to_features(actual_codes)
            archive = SourceImages(directory)
            native_locator = archive.add(native.decode(truth)[0])
                ...
                for seed in base['evaluation']['noise_seeds']:
                    ...
                    for name, model in models.items():
                        ...
                        if digest(received) != received_hash:
                            raise RuntimeError('receiver changed its actual observation')
                        choice = choices[name]
                        fresh_rows.append({'image_index': index, 'image_id': identifier, 'snr_db': snr, 'seed': seed,
                            'arm': name, 'variant': model.variant, 'training_policy': 'joint_E_R',
                            'available_updates': 5000, 'parent_updates': 7000, 'selected_step': choice['step'],
                            'selected_global_data_step': 7000 + choice['step'], 'checkpoint_sha256': choice['checkpoint_sha256'],
                            'encoder_sha256': choice['encoder_sha256'], 'selected_encoder_differs_from_parent': choice['selected_encoder_differs_from_parent'],
                            ...
                            'quality_origin': 'new_grid_measurement', 'quality_reference_sha256': '',
                            'online_TX_seconds': '', 'receiver_seconds': '', 'receiver_order_index': '',
            ...
            for name, model in models.items():
                ...
                fresh_noiseless.append({'image_index': index, 'image_id': identifier, 'arm': name,
                    ...
                    'quality_origin': 'new_grid_measurement', 'online_TX_seconds': '', 'receiver_seconds': '',
                    'latency_source': 'not_measured_in_this_quality_phase',
                    **feature_diagnostics(result, truth, 'continuous_mean'), **archive.add(image[0])})
            ...
            if anchor['image_id'] != identifier or any(abs(measured_native[key] - float(anchor[key])) > tolerance
                ...
                raise RuntimeError('the same-codec native quality anchor did not replay')
            clean = {**project_reference(anchor, references.receipt_sha), **native_locator,
                'native_anchor_policy': 'sealed_scores_verified_by_new_pixel_metric_replay'}
            ...
            if maximum_power_error > 1e-5:
                raise RuntimeError('new Grid waveform violated energy budget')
            ...
            write_json(source_receipt, {'image_id': identifier, 'new_quality_rows': len(fresh_rows),
                'maximum_power_error': maximum_power_error, 'original_reference_receipt_sha256': references.receipt_sha,
            ...
            status('EVALUATING_GRID_QUALITY_WITHOUT_LATENCY')
            print(f'Grid quality {completed}/100 rows={len(rows)}', flush=True)
        validate_rows(rows, config, original, reference, base)
        validate_selection(rows, milestone)
        validate_diagnostics(rows, clean_rows, noiseless_rows, support_rows, config, original, reference, base)
        validate_reference_diagnostics(noiseless_rows, support_rows, references)
        references.validate_reuse(rows)
        if (len(noiseless_rows) != evaluation['noiseless_rows'] or len(support_rows) != evaluation['support_rows'] or
            len(clean_rows) != evaluation['source_images']):
            raise RuntimeError('quality diagnostics or strong support controls are incomplete')
        after = {name: module_sha256(model) for name, model in objects.items()}
        if before != after:
            raise RuntimeError('quality evaluation changed model parameters')
        verify_sources(metadata['source_hashes'])
        for filename, values in (('per_frame.csv', rows), ('native_reference.csv', clean_rows),
            ('noiseless_mapping.csv', noiseless_rows), ('deep_support_supplement.csv', support_rows)):
            write_rows(output / filename, values)
        write_json(output / 'hardware.json', hardware_rows)
        status('GRID_QUALITY_COMPLETE_NOT_RESEARCH_COMPLETE')
        sessions = [json.loads(path.read_text()) for path in sorted((output / 'sessions').glob('*.json'))]
        terminal = {'GRID_QUALITY_COMPLETE_NOT_RESEARCH_COMPLETE', 'GRID_QUALITY_FAILED_OR_INTERRUPTED'}
        write_json(output / 'completion.json', {'status': 'GRID_QUALITY_COMPLETE', 'completed_local': now(), **bindings,
            'rows': len(rows), 'new_rows': sum(row['arm'] in new_names(config) for row in rows),
            'noiseless_rows': len(noiseless_rows), 'support_rows': len(support_rows), 'source_hashes': metadata['source_hashes'],
            'frozen_models_before': before, 'frozen_models_after': after, 'shared_start_requested': arguments.shared_gpu,
            'quality_only_no_current_timing': True, 'observed_process_hours_all_sessions': sum(row['observed_process_seconds'] for row in sessions) / 3600,
        ...
        status('GRID_QUALITY_FAILED_OR_INTERRUPTED', error=repr(error))
        raise
