...
def load_native():
    ...
    if not library.exists():
        ...
        (destination / "build.json").write_text(json.dumps({"source_sha256": source_hash, "command": command,
            "compiler": subprocess.check_output(["g++", "--version"], text=True).splitlines()[0],
            "library_sha256": hashlib.sha256(library.read_bytes()).hexdigest()}, indent=2) + "\n")
    descriptor = json.loads((destination / "build.json").read_text())
    if descriptor["source_sha256"] != source_hash or hashlib.sha256(library.read_bytes()).hexdigest() != descriptor["library_sha256"]:
...
def bit_marginal_table(log_prior):
    ...
    if logarithms.ndim != 2 or logarithms.shape[1] != 4096:
    ...
    for shift in range(11, -1, -1):
        ...
        table[:, selected] += np.log(mass[:, selected].sum(axis=1))[:, None]
        table[:, ~selected] += np.log(mass[:, ~selected].sum(axis=1))[:, None]
    return table
